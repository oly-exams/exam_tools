...
def moderation_detail(question_id, delegation_id, request=HttpRequest()):
    ...
    body =  render_to_string('ipho_marking/moderation_detail.html', context=ctx, request=request)
    ...
    file = open("html/test-{}-{}.html".format(question_id, delegation_id),"w")
    file.write(body[:start] + body[end:start2] + body[end2:])
    file.close()
    pdfkit.from_file("html/test-{}-{}.html".format(question_id, delegation_id), 'pdf/test-{}-{}.pdf'.format(question_id, delegation_id))


answer_sheets = Question.objects.filter(exam__hidden=False, exam__moderation_active=True, code='A')
