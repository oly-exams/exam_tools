...
def compile_blank():
    ...
    question_pdf = _compile_tex(body, [tex.TemplateExport(os.path.join(TEX_TEMPLATE_PATH, 'tex_resources', 'ipho2016.cls'))])
...
def compile_graph():
    ...
    body = render_to_string(os.path.join(TEX_TEMPLATE_PATH, 'tex', 'exam_graph.tex'), request=HttpRequest(), context=context)
    question_pdf = _compile_tex(body, [tex.TemplateExport(os.path.join(TEX_TEMPLATE_PATH, 'tex_resources', 'ipho2016.cls'))])
    bgenerator = iphocode.QuestionBarcodeGen(exam, question, student, qcode='W')
    page = pdf.add_barcode(question_pdf, bgenerator)
