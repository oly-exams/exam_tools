    ...
    for votingRight in VotingRight.objects.all():
        ...
        if r > 0.66:
            vote = Vote.objects.create(
                question=question, choice=choice1, voting_right=votingRight
            )
        elif r < 0.33:
            vote = Vote.objects.create(
                question=question, choice=choice2, voting_right=votingRight
            )
        else:
            vote = Vote.objects.create(
                question=question, choice=choice3, voting_right=votingRight
            )
            vote.save()
    return question.pk
