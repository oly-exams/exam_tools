        ...
        if os.path.isdir(config['event_folder_path']):
            ...
            for nav_file in nav_files:
                ...
                if os.path.isfile(abs_path):
                    ...
                    fs.append(f)
                    print('Override path: {}'.format(f.src_path))
                else:
                    fs.append(nav_file)
            for folder, __, nav_new_files in os.walk(config['event_folder_path']):
                for nav_new_file in nav_new_files:
                    nav_new_filepath = os.path.normpath(os.path.relpath(os.path.join(folder, nav_new_file), config['event_folder_path']))
...
if __name__ == '__main__':
    ...
    def builder():
        ...
        cfg = mk_config.Config(schema=mk_config.DEFAULT_SCHEMA, config_file_path=config_file_path)
