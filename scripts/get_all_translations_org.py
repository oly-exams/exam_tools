...
private_dirs_basepath = os.path.join(ROOT_DIR, 'media', 'downloads', 'delegations_private_dirs')
...
for private_dir in os.listdir(private_dirs_basepath):
    try:
        ...
        for pdf in os.listdir(pdfs_path):
            if not '_solution.pdf' in pdf and (f'_english_{private_dir[:3]}_.pdf' not in pdf or len(os.listdir(pdfs_path)) == 4):
