...
class SingleTableData(Data):
    ...
    def get_store_file_path(self, prefix=''):
        return os.path.join(self.store_folder, '{p}{i:03d}_{n}.json'.format(
            p=prefix, i=self.index, n=self.table_name))

    def get_source_file_path(self, prefix=''):
        return os.path.join(self.source_folder, '{p}{i:03d}_{n}.csv'.format(
            p=prefix, i=self.index, n=self.table_name))
    ...
    def load(self, prefix='', select=False, select_only=False):
        if select:
            with open(self.get_store_file_path(prefix), 'r') as stream:
                for ser_obj in django.core.serializers.deserialize(
                    'json', stream):
                    if (not select_only or not type(
                        ser_obj.object).objects.filter(**{
                            field.name: getattr(ser_obj.object, field.name)
                            for field in ser_obj.object._meta.fields}).exists()):
                        self.new_objects.append(
                            ObjectStore(ser_obj.object, True))
                        ser_obj.save()
        else:
            execute_subprocess(
    ...
    def dump(self, prefix='', select=False, select_only=False):
        if select and select_only:
            ...
            objs = itertools.chain(*[
                model.objects.all() for model in self.models])
        items = [x for x in objs if all(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-p', '--prefix', type=str,
        default='',
        help='Load from data with given prefix.')
    parser.add_argument(
    ...
    data_elements = [
        SingleTableData(
            0, django.contrib.auth.models.Permission,
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            1, django.contrib.auth.models.Group,
            source_folder=args.input_folder, store_folder=args.output_folder),
        ConditionSingleTableData(
            10, django.contrib.auth.models.User,
            'admins',
        ...
        ConditionSingleTableData(
            ...
            source_folder=args.input_folder, store_folder=args.output_folder),
        ConditionSingleTableData(
            20, ipho_core.models.Delegation,
            'delegation',
            (lambda x: x.name != 'Official'),
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            21, ipho_core.models.Student,
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            22, ipho_exam.models.Place,
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            31, ipho_exam.models.Exam,
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            32, ipho_exam.models.Question,
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            33, ipho_exam.models.Figure,
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            34, ipho_exam.models.VersionNode,
            source_folder=args.input_folder, store_folder=args.output_folder),
        ConditionSingleTableData(
            35, ipho_exam.models.TranslationNode,
            'delegation_translation',
            (lambda x: x.language.delegation.name != 'Official'),
            source_folder=args.input_folder, store_folder=args.output_folder),
        ConditionSingleTableData(
            36, ipho_exam.models.Language,
            'delegation_language',
            (lambda x: x.delegation.name != 'Official'),
            source_folder=args.input_folder, store_folder=args.output_folder),
        ConditionSingleTableData(
            37, ipho_exam.models.TranslationNode,
            'official_translation',
            (lambda x: x.language.delegation.name == 'Official'),
            source_folder=args.input_folder, store_folder=args.output_folder),
        SingleTableData(
            50, ipho_exam.models.ExamAction,
            source_folder=args.input_folder, store_folder=args.output_folder),

        MultiTableData(
            60, [ipho_marking.models.MarkingMeta, ipho_marking.models.Marking],
            'markings',
            source_folder=args.input_folder, store_folder=args.output_folder),
        ConditionMultiTableData(
            30, [ipho_core.models.Delegation, ipho_exam.models.Language],
            'official_delegation',
            [
                (lambda x: x.name == "Official"),
                (lambda x: x.delegation.name == 'Official')],
