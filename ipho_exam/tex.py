...
def html2tex(elem):  # pylint: disable=too-many-branches
    ...
    for sel in elem:  # pylint: disable=too-many-nested-blocks
            ...
            for att in list(sel.attrib.keys()):
                if att == "style":
                    if "font-style:italic" in sel.attrib[att]:
                        result.append("\\textit{%s}" % (html2tex(sel)))
        ...
        ## Emph
            ...
            result.append("\\emph{%s}" % (html2tex(sel)))
        ## Underline
        elif sel.tag in ["u"]:
            result.append("\\underline{%s}" % (html2tex(sel)))
        ## Subscript
        elif sel.tag in ["sub"]:
...
def html2tex_bs4(elem):  # pylint: disable=too-many-branches
    ...
    for sel in elem.children:  # pylint: disable=too-many-nested-blocks
        ...
        ## Subscript
            ...
            result.append("\\textsubscript{%s}" % (html2tex_bs4(sel)))
        ## Superscript
        elif sel.name in ["sup"]:
            result.append("\\textsuperscript{%s}" % (html2tex_bs4(sel)))
        elif sel.name in ["ul"]:
            result.append(
