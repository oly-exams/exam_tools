    ...
    for question in questions:
        for sl in student_languages:
            ...
            if question.is_answer_sheet() and question.working_pages > 0:
                ...
                body = render_to_string(os.path.join(TEX_TEMPLATE_PATH, 'tex', 'exam_blank.tex'), request=HttpRequest(),
                                                                                       context=context)
                compile_task = tasks.compile_tex.s(body, [tex.TemplateExport(os.path.join(TEX_TEMPLATE_PATH, 'tex_resources', 'ipho2016.cls'))])
...
def generate_extra_sheets(student, question, startnum, npages, template_name='exam_blank.tex'):
    ...
    body = render_to_string(os.path.join(TEX_TEMPLATE_PATH, 'tex', template_name), request=HttpRequest(),
                                                                                     context=context)
